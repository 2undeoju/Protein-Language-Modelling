...
# Utility function to pad a 2D tensor to the next multiple of chunk_size
# This is useful for ensuring that sequences are properly chunked
# when processing with mLSTM Attention

def pad_tensor_to_chunk(tensor, chunk_size=64, pad_value=0):
    seq_len = tensor.shape[1]
    remainder = seq_len % chunk_size
    if remainder == 0:
        return tensor
    pad_len = chunk_size - remainder
    padding = torch.full((tensor.size(0), pad_len), pad_value, dtype=tensor.dtype)
    return torch.cat([tensor, padding], dim=1)

#def pad_tensor_to_chunk(tensor, chunk_size=64, pad_value=0):
    #"""
    #Pads the 2D tensor [B, L] along dim=1 to the next multiple of chunk_size.
    #"""
